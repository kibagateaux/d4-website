...
const browsePosts = async ({ ghost, filter, tries = 5 } : browsePostsProps) : Promise<PostsOrPages | void>  => {
  try {
...
export const postsBrowserSelector = selectorFamily<PostsOrPages | void, GhostFilterPost>({
  key: 'postsBrowser',
  get: ({ limit = 2, ...filterProps}: GhostFilterPost) => async ({ get } : { get: GetRecoilValue }) : Promise<PostsOrPages | void>  => {
...
const readPosts = async ({ ghost, slug, tries = 5 } : readPostsProps) : Promise<PostOrPage | void>  => {
  try {
...
export const postsReadSelector = selectorFamily<PostOrPage | void, string>({
  key: 'postsRead',
  get: (slug: string) => async ({ get } : { get: GetRecoilValue }) : Promise<PostOrPage | void>  => {
    const ghost = get(ghostAtom)
    if (ghost) {
